...
export function ChartsCard({ balanceHistory, spendingByCategory }: ChartsCardProps) {
  const maxBalance = Math.max(...balanceHistory.map(item => item.balance))
  const maxSpending = Math.max(...spendingByCategory.map(item => item.amount))
    ...
    <Card variant="premium">
      <CardHeader>
        <div className="flex items-center space-x-3">
          ...
          <div>
            <CardTitle className="text-white">Análise e Gráficos</CardTitle>
            <p className="text-sm text-gray-400">Evolução do seu saldo e gastos</p>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        <div className="space-y-6">
          ...
          <div>
            <h4 className="text-white font-medium mb-4 flex items-center">
              ...
              Gastos por Categoria (Em Eventos)
            </h4>
          ...
          <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/10">
            <div className="text-center">
              <p className="text-2xl font-bold text-green-400">
                {balanceHistory[balanceHistory.length - 1]?.balance.toLocaleString('pt-BR') || '0'} TKT
              </p>
              <p className="text-sm text-gray-400">Saldo TKT Atual</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-red-400">
                R$ {spendingByCategory.reduce((sum, cat) => sum + cat.amount, 0).toLocaleString('pt-BR')}
              </p>
              <p className="text-sm text-gray-400">Total Gasto em Eventos</p>
            </div>
          </div>
        </div>
      </CardContent>
